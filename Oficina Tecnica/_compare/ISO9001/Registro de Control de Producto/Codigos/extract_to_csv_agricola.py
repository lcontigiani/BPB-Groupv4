...
def extract_row(code: str, data: dict) -> dict:
    ...
    # Dimensión A (hex entre caras): buscamos en dimensions
    a_dim = dims.get("A") or dims.get("hex_width") or {}
    row["A"] = format_number(a_dim.get("value_mm"))
    row["A+"] = to_um_str(a_dim.get("tol_plus_mm"))
    row["A-"] = to_um_str(a_dim.get("tol_minus_mm"))
    ...
    row["TORQUE DE SELLOS +"] = fmt_t(t_plus)
    row["TORQUE DE SELLOS -"] = fmt_t(t_minus)
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Genera CSV (Agrícola) desde JSON estándar.")
