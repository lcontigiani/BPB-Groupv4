...
def load_config(path: Path) -> Dict:
    ...
    if env_root:
        ...
        # Priority 2: Relative to config file. 
        # Config is usually in <Root>/Codigos/config.yaml, so Root is path.parent.parent
        # But we check if 'Codigos' is the parent to be safe, or just go one up.
        # Let's assume standard structure if config is in a subdir.
...
def main():
    ...
    # Solo procesamos el ÚLTIMO registro para evitar reprocesar historial antiguo innecesariamente
