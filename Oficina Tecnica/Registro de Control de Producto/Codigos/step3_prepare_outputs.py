...
def compute_aux_match_score(target: str, candidate: str) -> float:
    if not target or not candidate:
        return 0.0
    if candidate == target:
        return 100.0
    if abs(len(candidate) - len(target)) >= 20:
        return 0.0
    sm_ratio = difflib.SequenceMatcher(None, target, candidate).ratio()
    score = sm_ratio * 100.0

    def get_main_number(s: str) -> Optional[str]:
        nums = re.findall(r"\d+", s)
        if not nums:
            return None
        return max(nums, key=len)

    n1 = get_main_number(target)
    n2 = get_main_number(candidate)
    penalty = 0.0
    if n1 and n2 and n1 != n2:
        penalty += 50.0
    t_clean = target.strip().lower()
    c_clean = candidate.strip().lower()
    if len(target) >= 3 and len(candidate) >= 3:
        if target[:3] != candidate[:3]:
            penalty += 5.0
    prefix_same = False
    if '-' in target and '-' in candidate:
        prefix_a = target.rsplit('-', 1)[0]
        prefix_b = candidate.rsplit('-', 1)[0]
        ...
        if target != candidate:
            penalty += 20.0
    if c_clean not in t_clean and t_clean not in c_clean:
        penalty += 5.0 if prefix_same else 15.0
    score -= penalty
    if score < 0:
        score = 0.0
    return score
...
def collect_top_aux_matches(target: str, entries: List[Dict[str, str]], min_score: float = 65.0, limit: int = 3) -> List[Dict[str, Any]]:
    ...
    for row in entries:
        ...
        if not val:
            continue
        score = compute_aux_match_score(target, val)
        if score < min_score:
            continue
        key = (row.get("file") or "", val)
        if key in seen:
            continue
        seen.add(key)
        candidates.append({"score": score, "filename": row.get("file") or "", "match_val": val})
    candidates.sort(key=lambda x: x.get("score", 0), reverse=True)
    return candidates[:limit]


def _load_aux_file_rows(aux_dir: Path, filename: str, cache: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    if not filename:
        return {"headers": [], "rows": {}}
    if filename in cache:
        return cache[filename]
    info: Dict[str, Any] = {"headers": [], "rows": {}}
...
def enrich_aux_matches(matches: List[Dict[str, Any]], aux_dir: Path, file_cache: Dict[str, Dict[str, Any]]) -> List[Dict[str, Any]]:
    for m in matches:
        ...
        if not filename or not match_val:
            m["headers"] = m.get("headers", []) or []
            m["row"] = m.get("row", []) or []
            continue
        info = _load_aux_file_rows(aux_dir, filename, file_cache)
        m["headers"] = info.get("headers") or []
        m["row"] = (info.get("rows") or {}).get(match_val, [])
    return matches


def build_aux_top_matches_payload(rows: List[Dict[str, str]], entries: List[Dict[str, str]], aux_dir: Path, min_score: float = 65.0, limit: int = 3) -> Dict[str, Any]:
    ...
    for r in rows:
        ...
        if key in items:
            continue
        matches = collect_top_aux_matches(code, entries, min_score=min_score, limit=limit)
        direct_file = (r.get("Archivo_Auxiliar") or "").strip()
        direct_val = (r.get("Coincidencia") or "").strip()
        if direct_file and direct_val:
            direct = {"score": 100.0, "filename": direct_file, "match_val": direct_val}
            merged = [direct]
            seen = {(direct_file, direct_val)}
            for cand in matches:
                key2 = ((cand.get("filename") or ""), (cand.get("match_val") or ""))
                if key2 in seen:
                    continue
                merged.append(cand)
                seen.add(key2)
        ...
        matches = enrich_aux_matches(matches, aux_dir, file_cache)
        items[key] = {"code": code, "matches": matches}
    return {
        "version": 1,
...
def render_first_page_png(pdf_path: Path) -> Optional[bytes]:
    """
    Intenta renderizar la primera pÃ¡gina a PNG.
    1) Usa pypdfium2 (rÃ¡pido).
...
def extract_notes_with_gemini(pdf_path: Path) -> Optional[Dict]:
    ...
    prompt = (
        "Eres un extractor tÃ©cnico de planos de rodamientos. Devuelve SOLO JSON plano (sin markdown) con EXACTAMENTE estos campos y estructura (igual al ejemplo existente):\n"
        "{\n"
        "  \"purchase_order\": \"<numero OC si aparece>\",\n"
        "  \"code\": \"<codigo exacto del campo CÃ³digo/Code del rotulo, incluyendo prefijo numÃ©rico si existe ej 100-SAB211-55DG>\",\n"
        "  \"revision\": \"<letra o 0>\",\n"
        "  \"date\": \"dd/mm/yyyy\",\n"
        "  \"dimensions\": {\n"
        ...
        "- No inventes. Copia exactamente lo que veas (incluye PO y cÃ³digo marcado en pieza/rÃ³tulo).\n"
        "- Usa la Ãºltima fecha/revisiÃ³n si hay tabla de revisiones; incluye la lista completa en revision_history.\n"
...
def pick_latest_json_for_pdf(pdf: Path, json_dir: Path) -> Optional[Path]:
    """
    Busca el json de ese PDF en json_dir, eligiendo la revisiÃ³n mÃ¡s alta.
    Formatos esperados:
...
def adjust_code_with_provider(base_code: str, provider_code: str, factory_map: Dict[str, str]) -> str:
    """
    Reemplaza el sufijo (Ãºltimo segmento tras '-') por el cÃ³digo de fÃ¡brica (letras) si no coincide.
    provider_code: cÃ³digo numÃ©rico del JSON (ej: '195').
...
def generate_csv_from_json(product_code: str, data: Dict, tipo: str, out_path: Path) -> None:
    """
    Despacha al extractor correcto segÃºn tipo.
    """
    t = (tipo or "").lower()
...
def generate_csvs_for_po(po_dir: Path, factory_map: Dict[str, str]) -> None:
    ...
    Para cada PDF en la PO:
      - Elegir el JSON con mayor revisiÃ³n.
    ...
    for pdf in [p for p in po_dir.iterdir() if p.suffix.lower() == ".pdf"]:
        ...
        if not json_path or not json_path.exists():
            logging.warning("No se encontrÃ³ JSON para %s", pdf.name)
            continue
        try:
...
def main():
    parser = argparse.ArgumentParser(description="Paso 3: generar json/csv por PDF desde el Ãºltimo resumen.")
    ...
    try:
        fault_log = (cfg.get("log_file") or (Path(__file__).parent / "automation.log")).with_name("step3_fault.log")
        fault_log.parent.mkdir(parents=True, exist_ok=True)
        fault_fp = open(fault_log, "a", encoding="utf-8")
        fault_fp.write(f"\n=== RUN {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} PID={os.getpid()} ===\n")
        fault_fp.flush()
        faulthandler.enable(file=fault_fp, all_threads=True)
        logging.info("Faulthandler habilitado en %s", fault_log)
    except Exception as exc:
        logging.warning("No se pudo habilitar faulthandler: %s", exc)
    ...
    total_pdfs = 0
    for _po_dir in po_dirs:
        try:
            total_pdfs += len([p for p in _po_dir.iterdir() if p.suffix.lower() == ".pdf"])
        except Exception:
            pass
    progress = {"done": 0, "total": total_pdfs}
    ...
    for po_name in po_names:
        ...
        def _should_skip(pdf: Path, product_key: str) -> bool:
            if norm_str(product_key) in existing_codes:
                return True
            # Si ya existe algÃºn json para este PDF, no regenerar
            prefix = f"{pdf.stem}_notes_extracted_gemini"
            if any(json_dir.glob(f"{prefix}*.json")):
                return True
            return False

        # Concurrencia conservadora para reducir fallos nativos intermitentes en Step 3.
        try:
            max_workers = int(cfg.get("step3_max_workers", os.environ.get("STEP3_MAX_WORKERS", 4)))
        except Exception:
            max_workers = 4
        if max_workers < 1:
            max_workers = 1
        logging.info("Step 3 usando max_workers=%s", max_workers)
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as ex:
            ...
            for fut in as_completed(future_to_pdf):
                ...
                try:
                    ...
                    logging.warning("Fallo en %s con max_workers=%s: %s. Se agrupa para reintento secuencial.", pdf.name, max_workers, exc)
        ...
        if aux_dir and combined_rows:
            try:
                payload = build_aux_top_matches_payload(combined_rows, aux_cache, aux_dir, min_score=65.0, limit=3)
                out_top = aux_csv_dir / "aux_top_matches.json"
                out_top.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
            except Exception as exc:
                logging.warning("No se pudo generar aux_top_matches.json en %s: %s", aux_csv_dir, exc)
        ...
        # Inicializar approved_items vacÃ­o si no existe
