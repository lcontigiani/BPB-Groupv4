...
def extract_row(code: str, data: dict) -> dict:
    ...
    # Dureza: tipo HRc si hay datos de hardness_hrc
    ...
    # Dureza (tipo)
    has_hardness = any(v not in (None, "", "null", "-") for v in (inner_plus, inner_minus, outer_plus, outer_minus))
    row["Dureza"] = "HRc" if has_hardness else "NO"

    # Dureza+/-
    row["Dureza+"] = format_number(outer_plus) if outer_plus not in (None, "", "null", "-") else "NO"
    row["Dureza-"] = format_number(outer_minus) if outer_minus not in (None, "", "null", "-") else "NO"
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Genera CSV (Engranajes) desde JSON estándar.")
