...
def main() -> None:
    ...
    dest = model if inplace else (out_path or docs_root / "R019-04-salida.mpp")

    eventos = parse_csv(csv_path)
    if not eventos:
        raise ValueError("El CSV no tiene filas de eventos.")

    # Copia local para evitar problemas con rutas UNC y acentos
    tmp = Path(tempfile.gettempdir()) / "r01904-work.mpp"
